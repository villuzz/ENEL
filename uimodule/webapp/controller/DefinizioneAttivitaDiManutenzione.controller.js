sap.ui.define([
  ...
  return Controller.extend("PM030.APP1.controller.DefinizioneAttivitaDiManutenzione", {
      ...
      onSearchFilters: function () {
          ...
          var oBinding = this.byId("tbDefinizioneAttivitaDiManutenzione").getBinding("items");
          if (oBinding.isSuspended()) {
              oBinding.resume();
          }

          var filterArray = [];
          oData.Lingua.map((ling) => {
              filterArray.push(new sap.ui.model.Filter("Lingua", sap.ui.model.FilterOperator.EQ, ling));
          });
          oData.TPAttivitaPM.map((tap) => {
              filterArray.push(new sap.ui.model.Filter("TPAttivitaPM", sap.ui.model.FilterOperator.EQ, tap));
          });
          oData.DescrizioneTPAttivitaPM.map((dtpp) => {
              filterArray.push(new sap.ui.model.Filter("DescrizioneTPAttivitaPM", sap.ui.model.FilterOperator.EQ, dtpp));
          });
