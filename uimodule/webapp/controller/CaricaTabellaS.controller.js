sap.ui.define([
  ...
  return Controller.extend("PM030.APP1.controller.CaricaTabellaS", {
      onInit: function () {
          ...
          this._oTPC = new TablePersoController({table: this.byId("tbCaricaTabellaS"), componentName: "Piani", persoService: manutenzioneTable}).activate();
      },
      _onObjectMatched: async function () {
        var aT_PMO_S = await this._getTable("/T_PMO_S", []);
        var oModel = new sap.ui.model.json.JSONModel();
        oModel.setData(aT_PMO_S);
      ...
      onSearchFilters: function () {
          ...
          var oBinding = this.byId("tbCaricaTabellaS").getBinding("items");
          ...
          oData.NAttivita.map((na) => {
              filterArray.push(new sap.ui.model.Filter("NAttivita", sap.ui.model.FilterOperator.EQ, na));
          });
          oData.Classe.map((di) => {
              filterArray.push(new sap.ui.model.Filter("Divisa", sap.ui.model.FilterOperator.EQ, di));
          });
      ...
      handleNav: function(evt) {
        ...
        if (target) {
          var oTable = this.byId("tbCaricaTabellaS");
