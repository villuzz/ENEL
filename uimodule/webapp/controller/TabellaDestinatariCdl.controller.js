sap.ui.define([
  ...
  return Controller.extend("PM030.APP1.controller.TabellaDestinatariCdl", {
    onInit: function () {
      ...
      this.getView().setModel(oModelEnabled, "oModelEnabled");
    ...
    getValueHelp: async function () {
      ...
      sData.DIVISIONE = await this._getTableDistinct("/T_DEST", [], "Divisione");
      sData.DIVISIONENew = await this.Shpl("T001W", "CH");
      sData.CENTRO_LAVORO = await this._getTableDistinct("/T_DEST", [], "Arbpl");
      // sData.CENTRO_LAVORONew = await this.Shpl("ZPM4R_T_DEST", "SH");
      sData.DESTINATARIO = await this._getTableDistinct("/T_DEST", [], "Destinatario");
      sData.DESC_DESTINATARIO = await this._getTableDistinct("/T_DEST", [], "Txt");
      sData.RAGGRUPPAMENTO = await this._getTableDistinct("/T_DEST", [], "Raggruppamento");
      sData.RAGGRUPPAMENTONew = await this.Shpl("ZPM4R_H_RAG", "SH");
    ...
    // componiURL: function (line) {
    //   debugger
    //   var encode = encodeURIComponent(`Destinatario='${line.Destinatario}'`);
    //   var sURL = `/T_DEST(Werks='${line.Werks}',Arbpl='${line.Arbpl},` + encode + ')';
    //   return sURL;
    // },
      ...
      if (this.getView().byId("fileUploader").getValue() === "") {
        ...
        if (msg !== "") {
          ...
          for (i = 0; i < rows.length; i++) {
            var sDestinatari = this.DestinatariModel(rows[i]);
            if (sDestinatari.Werks.startsWith("C-")) { //Creazione 
              await this._saveHana("/T_DEST", sDestinatari);
            } else { // Modifica

              sURL = this.componiURLExcel(sDestinatari)
              await this._updateHana(sURL, sDestinatari);
            }
          }
